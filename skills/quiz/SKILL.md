...
You MUST call the AskUserQuestion tool to deliver each question. Do NOT write questions as plain text. Your response for each question must consist of a single AskUserQuestion tool call and nothing else — no text before or after it. One question per tool call. Wait for the user's answer before proceeding to the next.

**Multiple choice — call the AskUserQuestion tool with:**
- question: "Q1/5 (Multiple Choice) — Which Kubernetes object is responsible for ensuring a specified number of pod replicas are running at all times?"
- header: "Q1"
- multiSelect: false
- options:
  - A. Deployment (Manages rollouts and scaling of pods)
  - B. ReplicaSet (Ensures a stable set of replica pods)
  - C. DaemonSet (Runs a pod on every node)
  - D. StatefulSet (Manages stateful applications)

**True/False — call the AskUserQuestion tool with:**
- question: "Q2/5 (True or False) — In TCP, the receiver sends acknowledgments for every individual packet it receives."
- header: "Q2"
- multiSelect: false
- options:
  - True (Every packet gets its own ACK)
  - False (ACKs can cover multiple packets)

**Fill-in-the-blank — call the AskUserQuestion tool with:**
- question: "Q3/5 (Fill in the Blank) — The kubectl command to view all running pods in a namespace is: kubectl _____ pods -n <namespace>"
- header: "Q3"
- multiSelect: false
- options:
  ...
  - show (Display resource details)

**Short answer — call the AskUserQuestion tool with:**
- question: "Q4/5 (Short Answer) — Explain the difference between a Pod and a Deployment in Kubernetes. Pick the closest answer."
- header: "Q4"
- multiSelect: false
- options:
  - Pod = instance, Deployment = replicas (Pod is a single instance, Deployment manages replicas)
  - Pods = containers, Deployments = config (Pods run containers, Deployments are configuration files)
  - Other (None of these match my understanding)

**After the user answers, give feedback as text:**
...
Then immediately call the AskUserQuestion tool for the next question — no text before it other than the feedback from the previous question.
